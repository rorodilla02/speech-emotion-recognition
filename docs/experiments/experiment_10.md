# Experiment 10

| Field | Isi |
|-------|-----|
| ID | EXP-10 |
| Tanggal | 2026-08-03 |
| Skenario | RM2 fold_2 (Leave-One-Corpus-Out) |
| Data latih | RAVDESS + SAVEE |
| Data uji | TESS |
| Seed | 43 |
| Status | berhasil |

## Perbedaan terhadap baseline

Tidak ada. Konfigurasi training identik dengan RM1. Data validasi dibentuk
dari data latih memakai `ValidationSplitter` dengan seed tetap 42, sehingga
komposisinya sama pada seluruh seed.

## Hasil

| Metrik | Nilai |
|--------|-------|
| Epoch dijalankan | 26 |
| Epoch terbaik | 14 |
| Macro F1 validasi | 0.4783 |
| Accuracy uji | 0.2536 |
| Macro F1 uji | 0.2124 |
| Waktu per epoch | 7.6 detik |

## Perbandingan terhadap RM1

| | Nilai |
|---|---|
| Baseline RM1 pada TESS | 0.9986 |
| Rentang RM1 | 0,9976-1,0000 |
| Macro F1 RM2 | 0.2124 |
| Selisih | -0.7862 |

## Pengamatan

Tiga kelas terbanyak diprediksi: Surprise 41.3%, Fear 26.9%, Neutral 12.0%.
Konsentrasi dua kelas teratas 68.2%.
Kelas dengan F1 nol: Angry.

## Keputusan

Diterima sebagai hasil yang dilaporkan.

## Artefak

`data/models/rm2/fold_2/seed_43/`
